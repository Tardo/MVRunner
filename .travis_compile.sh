...
if [[ $TRAVIS_OS_NAME == 'linux' ]]; then
	...
	cmake -DCMAKE_BUILD_TYPE:STRING=$CMAKE_BUILD_TYPE -DZPG_PACKER_BIN="$ZPG_PACKER_BIN" ..
	make && make create_zpg
	cpack --debug --verbose -C CPackConfig.cmake
elif [[ $TRAVIS_OS_NAME == 'windows' ]]; then
	mkdir build/ && cd build/
    cmake -Werror=dev -G "$CMAKE_GENERATOR" -DCMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -DZLIB_ROOT="$ZLIB_ROOT" -DZPG_PACKER_BIN="$ZPG_PACKER_BIN" ..
    cd ..
